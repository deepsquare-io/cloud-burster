...
bin/cloud-burster-darwin-amd64: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o "$@" ./cmd/main.go

bin/cloud-burster-darwin-arm64: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o "$@" ./cmd/main.go

bin/cloud-burster-freebsd-amd64: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=freebsd GOARCH=amd64 go build -o "$@" ./cmd/main.go

bin/cloud-burster-freebsd-arm64: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=freebsd GOARCH=arm64 go build -o "$@" ./cmd/main.go
...
bin/cloud-burster-linux-arm64: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o "$@" ./cmd/main.go

bin/cloud-burster-linux-mips64: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=linux GOARCH=mips64 go build -o "$@" ./cmd/main.go

bin/cloud-burster-linux-mips64le: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=linux GOARCH=mips64le go build -o "$@" ./cmd/main.go

bin/cloud-burster-linux-ppc64: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=linux GOARCH=ppc64 go build -o "$@" ./cmd/main.go

bin/cloud-burster-linux-ppc64le: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=linux GOARCH=ppc64le go build -o "$@" ./cmd/main.go

bin/cloud-burster-linux-riscv64: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=linux GOARCH=riscv64 go build -o "$@" ./cmd/main.go

bin/cloud-burster-linux-s390x: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=linux GOARCH=s390x go build -o "$@" ./cmd/main.go

bin/cloud-burster-windows-amd64.exe: $(GO_SRCS) set-version
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o "$@" ./cmd/main.go
